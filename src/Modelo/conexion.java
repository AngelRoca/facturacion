...
public class conexion {
    ...
    public void actualizar(String tabla,String campos,String valores,String clausula){
        ...
        if(clausula==null)c="1";else c=clausula;
        for(int i=0;i<max;i++){
            aux+=camp[i]+"=?";
            if(i!=max-1)aux+=",";
        }
    ...
    public boolean prepararEstados(String query,String values){
        ...
        if(values!=null){
            ...
            pstm.close();
            }catch(Exception e){System.out.println("Problemas de sincronizacion con la base de datos:\n "+e);}
        }
        else{
            try{
            PreparedStatement pstm=(PreparedStatement)conectar().prepareStatement(query);
            r=pstm.execute();
            pstm.close();
            }catch(Exception e){System.out.println("PREPARAR ESTADOS. "+e);}
        }
        desconectar();
        return r;
    }
