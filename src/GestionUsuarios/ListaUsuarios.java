...
//obtenemos la cantidad de registros existentes en la tabla
        try {
            PreparedStatement pstm = (PreparedStatement) con.conectar().prepareStatement("SELECT count(1) as total FROM usuarios WHERE permisos=false ");
...
//realizamos la consulta sql y llenamos los datos en "Object"
        try {
            PreparedStatement pstm = (PreparedStatement) con.conectar().prepareStatement("SELECT "
                    + " nombre, password, sucursal "
                    + " FROM usuarios WHERE permisos=false");
            ResultSet res = pstm.executeQuery();
            int i = 0;
            while (res.next()) {
                String estNombre = res.getString("nombre");
                String estPassword = res.getString("password");
                String estSucursal = res.getString("sucursal");

                data[i][0] = estNombre;
                data[i][1] = estPassword;
                data[i][2] = estSucursal;
